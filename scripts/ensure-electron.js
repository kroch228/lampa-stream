...
// Gracefully skip when electron isn't installed (e.g. Vercel web-only deploy,
// or ELECTRON_SKIP_BINARY_DOWNLOAD is set). Avoids a hard crash on the missing
// require below.
if (process.env.ELECTRON_SKIP_BINARY_DOWNLOAD) {
  console.log("[ensure-electron] ELECTRON_SKIP_BINARY_DOWNLOAD set, skipping.");
  process.exit(0);
}
let electronPkg;
try {
  electronPkg = require("../node_modules/electron/package.json");
} catch {
  console.log("[ensure-electron] electron package not present, skipping (web-only build?).");
  process.exit(0);
}
