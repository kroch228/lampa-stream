...
import TorrPlayer from "../components/TorrPlayer";
...
// Expandable episode description (memoized — props are stable strings, so it
// skips re-renders when its parent EpisodeCard re-renders for unrelated state).
const EpisodeDesc = memo(function EpisodeDesc({ overview, episodeName }) {
...
export default function TVPage({
  ...
  const [playerSource, setPlayerSource] = useState(() => {
    ...
    if (!!window.__webShim) {
      if (saved && saved !== "online") storage.set("playerSource", "online");
      return "online";
    }
    return saved || NON_ANIME_DEFAULT_SOURCE;
  });
  // On web, webview never loads → never show "Loading Vidking…".
  useEffect(() => {
    if (!!window.__webShim) setWebviewLoading(false);
  }, [!!window.__webShim]);
  ...
  // AniSkip
  const [skipTimings, setSkipTimings] = useState(null); // { intro?, outro?, recap? }
  const [skipPrompt, setSkipPrompt] = useState(null); // "intro" | "outro" | "recap" | null
  ...
  useEffect(() => {
    ...
    if (!!!window.__webShim) setWebviewLoading(true); // instantly blank the player on every source/episode switch (web has no <webview>)
  }, [
    item.id,
  ...
  useEffect(() => {
    let mounted = true;
    // On web, the ONLY source is "online" — never switch away from it.
    if (!!window.__webShim) return;
    setAnilistData(null);
  ...
  // Show loader instantly when playback starts (desktop only — on web there is
  // no <webview> to load, so the overlay would never clear; OnlinePlayer has
  // its own loading state for the "online" source).
  useEffect(() => {
    if (playing && !!!window.__webShim) setWebviewLoading(true);
  }, [playing]);
      ...
      interval = setInterval(async () => {
        try {
          ...
          if (aniSkipActive && result?.currentTime != null) {
            ...
            const inRecap =
              recap && ct >= recap.startTime && ct < recap.endTime - 1;
            const inIntro =
              intro && ct >= intro.startTime && ct < intro.endTime - 1;
            const inOutro =
              outro && ct >= outro.startTime && ct < outro.endTime - 1;
            // Recap first: "previously on..." plays before the OP, so prioritize
            // it over intro when both somehow overlap.
            const activeSegment = inRecap
              ? "recap"
              : inIntro
                ? "intro"
                : inOutro
          ...
          if (result && result.duration > 0) {
            ...
            // Autoplay trigger: start the next-episode countdown when the
            // ending/outro begins (if we have an outro timing), or — as a
            // fallback when no outro is known — when only `watchedThreshold`
            // seconds remain. Fires once per episode (localCountdownStartedRef).
            const outroStarted =
              skipTimings?.outro && ct >= skipTimings.outro.startTime;
            if (
              (outroStarted || remaining <= watchedThreshold) &&
          ...
          {playing && selectedEp && (
            <div className="section">
              ...
              <div
                ...
                ref={playerWrapRef}
              >
                {sourceIsInApp(playerSource) && (
                  <div className="torrplayer-stage">
                    {playerSource === "online" ? (
                      <OnlinePlayer
                        apiKey={apiKey}
                        item={item}
                        ...
                        onNext={triggerAutoplay}
                      />
                    ) : (
                      <TorrPlayer
                        queryTitle={`${item.name || item.title || ""} S${String(selectedSeason).padStart(2, "0")}E${String(selectedEp?.episode_number || 0).padStart(2, "0")}`}
                        isTv={true}
                        season={selectedSeason}
                        episode={selectedEp?.episode_number}
                        title={`${item.name || item.title} S${selectedSeason}E${selectedEp?.episode_number ?? ""}`}
                        progressKey={currentProgressKey}
                        saveProgress={saveProgress}
                        onMarkWatched={onMarkWatched}
                        watchedThreshold={watchedThreshold}
                ...
                {webviewLoading && !!!window.__webShim && !resolveError && !sourceIsInApp(playerSource) && (
                  <div
                    style={{
                      position: "absolute",
                      inset: 0,
                  ...
                  return epPct > 0 || hasMarkers ? (
                    <div className="progress-bar-row">
                      ...
                      >
                        ...
                        {/* AniSkip recap marker (blue, to distinguish from intro/outro) */}
                        {dur > 0 && skipTimings?.recap && (
                          <div
                            title="Recap"
                            style={{
                              position: "absolute",
                              top: 0,
                              left: `${(skipTimings.recap.startTime / dur) * 100}%`,
                              width: `${((skipTimings.recap.endTime - skipTimings.recap.startTime) / dur) * 100}%`,
                              height: "100%",
                              background: "rgba(99,149,255,0.75)",
                              borderRadius: 2,
                              pointerEvents: "none",
                            }}
                          />
