...
export default function MoviePage({
  ...
  const [playerSource, setPlayerSource] = useState(() => {
    ...
    // On web, ONLY "online" works. Check window.__webShim directly (set by
    // the web shim at import time) — more reliable than a re-exported const.
    const isWeb = typeof window !== "undefined" && !!window.__webShim;
    if (isWeb) {
  ...
  useEffect(() => {
    ...
    if (!!!window.__webShim) setWebviewLoading(true); // instantly blank the player on every source/item switch (web has no <webview>)
  ...
  useEffect(() => {
    ...
    if (sourceIsAsync(playerSource) && !!!window.__webShim) {
  ...
  // Show loader instantly when play starts (desktop only — on web there is no
  // <webview> to load, so the overlay would never clear; OnlinePlayer has its
  // own loading state for the "online" source).
  useEffect(() => {
            ...
            {sourceIsInApp(playerSource) && (
              <div className="torrplayer-stage">
                {playerSource === "online" ? (
                  ...
                  <TorrPlayer
                    queryTitle={`${item.title || ""} ${details?.release_date?.slice(0, 4) || ""}`.trim()}
                    isTv={false}
                    title={item.title}
                    progressKey={progressKey}
                    saveProgress={saveProgress}
