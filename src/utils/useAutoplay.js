...
export function useAutoplay({ nextEp, playEpisode, restricted }) {
  ...
  const triggerAutoplay = useCallback(() => {
    ...
    // Instant mode: skip the countdown/overlay and play the next episode
    // immediately when the ending trigger fires (outro start or threshold).
    const instant = storage.get(STORAGE_KEYS.AUTOPLAY_NEXT_INSTANT) ?? false;
    if (instant) {
